...
export default function Home() {
  ...
  async function searchFlights() {
    setMessage(""); setOffers([]); if(!origin||!destination||!departure){setMessage("Enter origin, destination and departure date.");return;} if(trip==="Round trip"&&!returnDate){setMessage("Choose a return date for a round trip.");return;} setSearching(true); try { const res=await fetch("/api/flights/search",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tripType:trip==="Round trip"?"round_trip":trip==="One way"?"one_way":"multi_city",origin,destination,departureDate:departure,returnDate,travellers,cabin})}); const data=await res.json(); if(!res.ok) throw new Error(data.error||"Flight search failed."); setOffers(data.offers||[]); setMessage(data.offers?.length?`${data.offers.length} flight offers found.`:"No flights found."); } catch(e:any){setMessage(e.message||"Unable to search flights.");} finally{setSearching(false);}
  }
        ...
        <section className="hero">
          <div className="container">
            ...
            <div className="searchCard" id="flights">
              ...
              {message && <p style={{color:"#123b65",margin:"14px 2px 0",fontSize:13}}>{message}</p>}{offers.length>0&&<div className="liveResults"><h2>Available flights</h2><div className="cards">{offers.map((o:any)=><article className="card flightResult" key={o.id}><strong>{o.owner?.name||o.owner?.iata_code||"Airline"}</strong><div className="routeMeta">{o.slices?.map((s:any)=>s.segments?.map((g:any)=>g.operating_carrier?.name||g.marketing_carrier?.name||"Flight").join(" → ")).join(" · ")}</div><div className="price">{o.total_currency} {o.total_amount}</div><button className="cardAction" onClick={async()=>{const r=await fetch("/api/flights/revalidate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({offer_id:o.id})});const d=await r.json();if(r.ok){setMessage("Fare revalidated. Continue to passenger details from your account.");window.location.href="/dashboard?offer="+encodeURIComponent(o.id)}else setMessage(d.error||"This fare is no longer available.");}}>Select flight <ArrowRight size={15} style={{verticalAlign:"middle"}}/></button></article>)}</div></div>}
            </div>
          </div>
        </section>
